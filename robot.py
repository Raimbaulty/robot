...
class Robot(Job):
    ...
    def __init__(self, config: Config, wcf: Wcf, chat_type: int) -> None:
        ...
        self.commands = {
            ...
            "扮演": self.handle_扮演, "点歌": self.handle_点歌, "摸鱼": self.handle_摸鱼,
    ...
    def 点歌台(self, msg, chat, wxid):
        type_status = 0
        msg_content = msg.content.replace('点歌', "").strip()   
        if not USER_MUSIC_STATS.get(msg.sender + msg.roomid): 
            print('开始点歌')
            # 数据 状态类型 0 txt 1 xml                        
            USER_MUSIC_STATS[msg.sender + msg.roomid] = 'internet'
            url = 'https://www.yyfoam.top/music_api'
            data = {
                'from_uid': f'{msg.sender + msg.roomid}',
                'detail': {'content': f'/song {msg_content}'}
            }
            response = requests.post(url, json=data)
            rsp = response.json().get('msg')
            try:
                if rsp.get('xml_data'):
                    rsp = rsp.get('xml_data')
                    type_status = 1
            except AttributeError:
                rsp = '\n点歌台>联网模式\n\n说明：该模式速度可能会慢一些\n\n输入 0 退出\n\n' + rsp           
        else:
            if USER_MUSIC_STATS.get(msg.sender + msg.roomid) == 'internet':
                if msg_content == '0':
                    del USER_MUSIC_STATS[msg.sender + msg.roomid]
                    del USER_MUSIC_DATA[msg.sender + msg.roomid]
                    rsp = help_
                else:
                    url = 'https://www.yyfoam.top/music_api'
                    data = {
                        'from_uid': f'{msg.sender + msg.roomid}',
                        'detail': {'content': msg_content},
                        'sender': wxid
                            }
                    response = requests.post(url, json=data)
                    rsp = response.json().get('msg')
                ...
                if msg_content == '0':
                    ...
                    try:
                        rsp = USER_MUSIC_DATA[msg.sender + msg.roomid][msg_content]
                        type_status = 1
                    except KeyError:
                        rsp = '当前位置>点歌>联网模式\n\n输入 0 退出\n\n序号错误,请从新选择！'
        return rsp, type_status
    ...
    def handle_画(self, msg) -> None:
        # 调用Dall-E绘画，格式：画 一只会飞的猪
        content = msg.content.replace('画', "").strip()
        if not content:
            return f"1.功能介绍：\nDall-E绘画\n2.调用格式：\n画一只会飞的猪"        
        prompt = content
        url = "https://dalle.daosuijun.top/v1/images/generations"
        ...
        data = response.json()
        print(data)
        image = data.get('data', [{}])[0].get('url', 'URL not found')
        print(image)
        return image
    ...
    @add_receiver_info
    def handle_点歌(self, msg):
        # 进入点歌台模式，点歌格式为：点歌 菊花台 暂不可用
        rsp, type_status = self.点歌台(msg, self.chat, self.wxid)
        if rsp:
            if type_status == 0:                     
                return rsp
            else:                            
                return rsp
                # self.sendTextMsg('歌曲已发送，已退出点歌模式', msg.roomid, msg.sender) 
                del USER_MUSIC_STATS[msg.sender + msg.roomid]
                try:
                    del USER_MUSIC_DATA[msg.sender + msg.roomid]
                except KeyError:
    ...
    # def tts(self, msg): #百度版
    #     #根据文字调用百度tts生成语音，已挂
    #     choose = 3 #选择转音频的人物，女生1 输入0 ｜ 女生2输入：5｜男生1 输入：1｜男生2 输入：2｜男生3 输入：3
    #     su = 50   #语速，无需必填，默认即可
    #     yd = 5     #音调，默认正常，1～5调整
    #     params = {'msg':msg, 'choose':choose, 'su':su, 'yd':yd}
    #     try:
    #         response = requests.get('https://zj.v.api.aa1.cn/api/baidu-01', params=params)
    ...
    #                 print('未能获取语音下载链接')
    #         else:
    #             print(f'生成语音失败: {data["msg"]}')
    #     except requests.exceptions.RequestException as e:
    #         print(f'Error making TTS request: {e}')
    #     return None  

    # def tts(self, msg): #ChatGPT版
    # #根据文字调用ChatGPT生成语音
    # speaker = 'shimmer'
    # key = 'sess-uuitEKlbFJmRYDMuyB7SCEtP9o0khvYAVZ79heHA'
    # url = 'api.openai-proxy.com'
    # params = {'msg':msg, 'Speaker':speaker, 'key':key, 'url':url}
    # print(params)                   
    # response = requests.get('https://api.lolimi.cn/API/yyhc/gpt.php', params = params)
    # if response.status_code == 200:                   
    #     with open(r'C:\Users\Raimbault\WeChatRobot-39.0.5.0\audio.mp3', 'wb') as audio_file:
